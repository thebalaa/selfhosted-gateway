...
link:
	docker run -e SSH_AGENT_PID=$$SSH_AGENT_PID -e SSH_AUTH_SOCK=$$SSH_AUTH_SOCK -v $$SSH_AUTH_SOCK:$$SSH_AUTH_SOCK --rm fractalnetworks/gateway-cli:latest $(GATEWAY) $(FQDN) $(EXPOSE)
